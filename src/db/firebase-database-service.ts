...
export class FirebaseDatabaseService implements DatabaseService {
  ...
  constructor(private config?: { serviceAccountPath?: string; serviceAccountJson?: Record<string, unknown>; databaseURL?: string }) {
    try {
      ...
      if (!admin.apps || admin.apps.length === 0) {
        const opts: any = {};
        if (this.config?.serviceAccountJson) opts.credential = admin.credential.cert(this.config.serviceAccountJson);
        else if (this.config?.serviceAccountPath) opts.credential = admin.credential.cert(require(this.config.serviceAccountPath));
        if (this.config?.databaseURL) opts.databaseURL = this.config.databaseURL;
      ...
      console.warn('FirebaseDatabaseService: firebase-admin not initialized', message);
    }
  }

  private ensure() {
    if (!this.db) throw new Error('Firebase not initialized. Install and configure firebase-admin or provide service account.');
  }

  private collection(name: string) { this.ensure(); return this.db.collection(name); }
  ...
  async getAllPitEntries(year?: number, eventCode?: string, competitionType?: CompetitionType): Promise<PitEntry[]> {
    ...
    if (competitionType !== undefined) q = q.where('competitionType', '==', competitionType);
  ...
  async checkPitScoutExists(teamNumber: number, eventCode: string): Promise<boolean> {
    const ref = this.collection('pitEntries');
    const snap = await ref.where('teamNumber', '==', teamNumber).where('eventCode', '==', eventCode).limit(1).get();
    return !snap.empty;
  }
  ...
  async checkMatchScoutExists(teamNumber: number, matchNumber: number, eventCode: string): Promise<boolean> {
    const ref = this.collection('matchEntries');
    const snap = await ref.where('teamNumber', '==', teamNumber)
  ...
  async getAllCustomEvents(year?: number, competitionType?: CompetitionType): Promise<CustomEvent[]> {
    ...
    if (competitionType !== undefined) q = q.where('competitionType', '==', competitionType);
    const snap = await q.get();
    return snap.docs.map((d: any) => ({ ...(d.data()), id: undefined } as CustomEvent));
  }
  ...
  async getPicklistByEvent(eventCode: string, year: number, competitionType: string, picklistType?: string): Promise<any | undefined> {
    let q: any = this.collection('picklists').where('eventCode', '==', eventCode).where('year', '==', year).where('competitionType', '==', competitionType);
  ...
  async getPicklistsByEvent(eventCode: string, year: number, competitionType: string): Promise<any[]> {
    const snap = await this.collection('picklists').where('eventCode', '==', eventCode).where('year', '==', year).where('competitionType', '==', competitionType).get();
  ...
  async getPicklistEntries(picklistId: number): Promise<any[]> {
    const snap = await this.collection('picklistEntries').where('picklistId', '==', picklistId).orderBy('rank', 'asc').get();
    return snap.docs.map((d: any) => d.data());
  }
  ...
  async updatePicklistEntryRank(picklistId: number, teamNumber: number, rank: number): Promise<void> {
    const ref = this.collection('picklistEntries');
    const snap = await ref.where('picklistId', '==', picklistId).where('teamNumber', '==', teamNumber).limit(1).get();
    if (snap.empty) return;
  ...
  async reorderPicklistEntries(picklistId: number, entries: Array<{teamNumber: number, rank: number}>): Promise<void> {
    ...
    for (const e of entries) {
      const snap = await ref.where('picklistId', '==', picklistId).where('teamNumber', '==', e.teamNumber).limit(1).get();
      if (!snap.empty) {
        batch.update(snap.docs[0].ref, { rank: e.rank, updated_at: new Date() });
      }
    }
    await batch.commit();
  }
  ...
  async getPicklistNotes(picklistId: number, teamNumber?: number): Promise<any[]> {
    let q: any = this.collection('picklistNotes').where('picklistId', '==', picklistId);
  ...
  async resetDatabase(): Promise<void> {
    const collections = ['pitEntries','matchEntries','picklists','picklistEntries','picklistNotes','customEvents'];

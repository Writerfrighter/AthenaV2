...
import {
  getCombinedAlliancePosition,
  parseCombinedAlliancePosition,
} from "./match-form-utils";
...
export function MatchInfoSection({
  ...
  useEffect(() => {
    ...
    if ((matchChanged || allianceChanged || positionChanged) && matchNum > 0) {
      const autoTeam = getTeamForPosition(
        matchNum,
        currentAlliance,
        currentPosition,
      );
  ...
  const isAutoAssigned = (() => {
    ...
    const expectedTeam = getTeamForPosition(
      matchNum,
      formData.alliance,
      formData.alliancePosition || 1,
    );
    ...
    <Card className="border rounded-xl shadow-sm">
      ...
      <CardContent className="grid grid-cols-1 sm:flex sm:justify-between gap-4 sm:gap-10">
        <div className="space-y-2 w-full">
          <div className="flex items-center gap-2 h-5">
            ...
            {isMatchFromScoutingSchedule && (
              <Badge
                variant="secondary"
                className="text-xs flex items-center gap-0.5 bg-amber-500/10 text-amber-600 dark:text-amber-400 py-0 h-5"
              >
        ...
        <div className="space-y-2 w-full">
          <div className="flex items-center gap-2 h-5">
            ...
            {isAutoAssigned && (
              <Badge
                variant="secondary"
                className="text-xs flex items-center gap-0.5 bg-primary/10 text-primary py-0 h-5"
              >
          ...
          {eventTeamNumbers.length > 0 ? (
            <Select
              value={
                formData.teamNumber === 0
                  ? undefined
                  : String(formData.teamNumber)
              }
              onValueChange={(value) =>
                onBasicInputChange("teamNumber", Number(value))
              }
            >
              <SelectTrigger
                className={
                  isAutoAssigned
                    ? "border-primary/50 bg-primary/5 w-full"
                    : "w-full"
                }
              >
        ...
        <div className="space-y-2 w-full">
          ...
          <Select
            value={getCombinedAlliancePosition(
              formData.alliance,
              formData.alliancePosition || 1,
            )}
            onValueChange={(value) => {
              const { alliance, position } =
                parseCombinedAlliancePosition(value);
          ...
          >
            <SelectTrigger
              className={
                isAllianceFromScoutingSchedule
                  ? "border-amber-500/50 bg-amber-500/5 w-full"
                  : "w-full"
              }
            >
              <SelectValue />

...
export function PicklistConfig({
  ...
  const initializePicklistFromRanking = useCallback(async () => {
    ...
    try {
      ...
      toast.success(
        `${activeTab} picklist created with ${entriesToCreate.length} teams`,
      );
    } catch (error) {
          ...
          <Card>
            <CardHeader>
              <div className="flex items-center justify-between">
                ...
                <Badge
                  variant={activePiclist.picklist ? "default" : "secondary"}
                >
              ...
              {!activePiclist.picklist ? (
                <div className="space-y-4">
                  <p className="text-sm text-muted-foreground">
                    No picklist created yet. Initialize from qualification
                    rankings:
                  </p>
                  <Button
                    onClick={initializePicklistFromRanking}
                    className="w-full"
                  >
                    ...
                    {activeTab === "main"
                      ? "Picklist"
                      : `${activeTab.toUpperCase()} Picklist`}
                  </Button>
                  {activePiclist.initialRanking.length > 0 && (
                    <div className="mt-4 space-y-2">
                      <p className="text-xs text-muted-foreground">
                        Initial rankings preview:
                      </p>
                      <div className="max-h-96 overflow-y-auto space-y-1">
                        {activePiclist.initialRanking
                          .slice(0, 10)
                          .map((team) => (
                            <div
                              key={team.teamNumber}
                              className="flex justify-between text-xs p-2 hover:bg-accent rounded"
                            >
                ...
                <div className="space-y-2 max-h-96 overflow-y-auto">
                  {activePiclist.entries
                    ...
                    .map((entry) => (
                      ...
                      >
                        <div className="flex-shrink-0 font-bold text-lg w-8">
                          #{entry.rank}
                        </div>
                ...
                <>
                  ...
                  <Dialog>
                    ...
                    <DialogContent className="max-w-2xl max-h-96 overflow-y-auto">
                      <DialogHeader>
                        <DialogTitle>
                          Team {selectedTeam} - Scouting Notes
                        </DialogTitle>
                        <DialogDescription>
                          Qualitative notes collected from pit and match
                          scouting
                        </DialogDescription>
                      </DialogHeader>
                      <div className="space-y-4">

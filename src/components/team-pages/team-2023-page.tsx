...
export function Team2023Page({ teamNumber }: Team2023PageProps) {
  ...
  // Query 2023 specific data
  const matchEntries = useLiveQuery(() => 
    db.matchEntries
      .where('teamNumber')
      .equals(teamNumber)
      .and(entry => entry.year === 2023)
      .toArray()
  );
  ...
  const calculateChargedUpStats = () => {
    ...
    return {
      avg_auto_cones: totals.auto_cones / count,
      avg_auto_cubes: totals.auto_cubes / count,
      avg_teleop_cones: totals.teleop_cones / count,
      avg_teleop_cubes: totals.teleop_cubes / count,
      avg_total: (totals.auto_cones + totals.auto_cubes + totals.teleop_cones + totals.teleop_cubes) / count
    };
  };

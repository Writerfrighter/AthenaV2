...
const rebuiltConfig: YearConfig = {
  competitionType: 'FRC',
  gameName: 'REBUILT',
  startPositions: ['Left Trench', 'Left Bump', 'Hub', 'Right Bump', 'Right Trench'],
  pitScouting: {
    autonomous: {},
    teleoperated: {},
    endgame: {}
...
async function runTest() {
  ...
  try {
    ...
    if (userNameMap.size > 0) {
      ...
      console.log('⚠️  Could not fetch user names (auth required?), displaying user IDs');
    }
    ...
    const result = computeScouterAccuracy(matches, officialResults, rebuiltConfig, {
      verbose: true,
    ...
    result.scouters.forEach((scouter, index) => {
      ...
      const name = resolveName(userNameMap, scouter.scouterId).substring(0, 16).padEnd(16);
    ...
    if (result.scouters.length >= 4) {
      ...
      console.log(`✓ Top 25% performers: ${topQuarter.map(s => resolveName(userNameMap, s.scouterId)).join(', ')}`);

      const bottomQuarter = result.scouters.slice(Math.ceil(result.scouters.length * 0.75));
      console.log(`⚠️  Bottom 25% (needs help): ${bottomQuarter.map(s => resolveName(userNameMap, s.scouterId)).join(', ')}`);
    }
      ...
      for (const [matchNum, alliances] of Array.from(matchMap.entries()).sort((a, b) => a[0] - b[0])) {
        ...
        for (const eq of alliances.sort((a, b) => a.alliance.localeCompare(b.alliance))) {
          ...
          if (eq.skipped) {
            ...
            console.log(`         Robots: ${eq.robotCount}/${eq.expectedRobots} | Scouters: ${eq.scouterIds.map(s => resolveName(userNameMap, s)).join(', ') || 'none'}`);
          } else {
            ...
            console.log(`         Robots: ${eq.robotCount}/${eq.expectedRobots} | Scouters: ${eq.scouterIds.map(s => resolveName(userNameMap, s)).join(', ')}`);

/**
 * Test script for SPR (Scouter Performance Rating) on FTC Capek League Tournament
 *
 * This script validates the SPR algorithm using real FTC match data from USWAMVSQ2 event
 * with actual scouter userIds and official results from the FTC Events API.
 *
 * USAGE:
 *   pnpm tsx test-ftc-spr-capek.ts
 *
 * WHAT IT TESTS:
 ...
 *   - Event USWAMVSQ2 must have match entries in database
 */
...
const EVENT_CODE = "USWAMVSQ2"; // Capek League Tournament
const SEASON = 2026;
...
async function runTest() {
  ...
  try {
    ...
    result.scouters.forEach((scouter, index) => {
      ...
      let indicator = "  ";
      if (scouter.percentile >= 75)
        indicator = "🌟"; // Top 25%
      else if (scouter.percentile >= 50)
        indicator = "✓ "; // Above average
      else if (scouter.percentile >= 25)
        indicator = "⚠️ "; // Below average
    ...
    if (result.scouters.length >= 4) {
      ...
      console.log(
        `✓ Top 25% performers: ${topQuarter.map((s) => s.scouterId).join(", ")}`,
      );
      ...
      console.log(
        `⚠️  Bottom 25% (needs help): ${bottomQuarter.map((s) => s.scouterId).join(", ")}`,
      );
    }
      ...
      for (const [matchNum, alliances] of Array.from(matchMap.entries()).sort(
        ...
        for (const eq of alliances.sort((a, b) =>
          ...
          if (eq.skipped) {
            ...
            console.log(
              `         Robots: ${eq.robotCount}/${eq.expectedRobots} | Scouters: ${eq.scouterIds.map((s) => s.substring(5, 15)).join(", ") || "none"}`,
            );
          } else {
            ...
            console.log(
              `         Robots: ${eq.robotCount}/${eq.expectedRobots} | Scouters: ${eq.scouterIds.map((s) => s.substring(5, 15)).join(", ")}`,
            );
            console.log(
